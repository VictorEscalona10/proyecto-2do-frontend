...
export function OrderPage() {
  ...
  const getOrdersByIdentification = async (identification) => {
    ...
    try {
      ...
      const filteredOrders = allOrders.filter((order) => {
        ...
        return (
          userIdentification && userIdentification.toString() === identification
        );
      });

      console.log(
        `Encontradas ${filteredOrders.length} órdenes para cédula: ${identification}`,
      );
  ...
  const searchOrders = () => {
    if (!searchTerm.trim()) {
      setError(
        `Por favor ingresa un ${searchType === "email" ? "email" : "cédula"} válido`,
      );
      return;
    }
  ...
  const updateOrderStatus = async (orderId, newStatus, userName) => {
    showModal(
      `¿Estás seguro de cambiar el estado de la orden #${orderId} a "${getStatusText(newStatus)}"?`,
      "confirm",
      async () => {
        try {
          setLoading(true);
        ...
        <div className="orders-list">
          <div className="orders-header">
            ...
            <div className="orders-meta">
              ...
              <button onClick={toggleAllOrders} className="toggle-all-btn">
                {expandedOrders.size === orders.length
                  ? "Contraer Todas"
                  : "Expandir Todas"}
              </button>
            </div>
          ...
          {orders.map((order) => {
            ...
            return (
              ...
              >
                ...
                {isExpanded && (
                  <div
                    className="order-expandable-content"
                        ...
                        {order.orderDetails.map((detail) => (
                          <div key={detail.id} className="order-item">
                            <div className="item-info">
                              <p className="item-name">{detail.product.name}</p>
